...
class AttachmentSectionPreviewProvider : PreviewParameterProvider<AttachmentSectionInput> {
    ...
    override val values: Sequence<AttachmentSectionInput>
        get() = sequenceOf(
            ...
            createAttachmentSectionInput(
                ...
                loadingFile = createFile(AttachmentId("1"), "file1", AttachmentType.Calendar).some()
            ),
            createAttachmentSectionInput(
                files = emptyList()
            )
    ...
    private fun createAttachmentSectionInput(files: List<Attachment>, loadingFile: Option<Attachment> = None) =
        AttachmentSectionInput(files = files, loadingFile = loadingFile)
}
