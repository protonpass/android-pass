...
@Composable
fun AddFolderToVaultDialog(
    modifier: Modifier = Modifier,
    onNavigate: (VaultNavigation) -> Unit,
    viewModel: AddFolderToVaultViewModel = hiltViewModel()
) {
    LaunchedEffect(Unit) {
        viewModel.onStart()
    }
    ...
    LaunchedEffect(state.event) {
        if (state.event == AddFolderToVaultEvent.FolderAdded) {
    ...
    AddFolderToVaultDialogContent(
        ...
        onCreate = { viewModel.onAddFolder() },
