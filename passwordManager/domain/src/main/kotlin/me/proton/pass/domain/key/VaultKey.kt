package me.proton.pass.domain.key
...
fun VaultKey.publicKey(cryptoContext: CryptoContext): PublicKey =
    when (key) {
...
@Suppress("TooGenericExceptionThrown")
inline fun <R> VaultKey.usePrivateKey(context: CryptoContext, block: KeyHolderContext.() -> R): R =
    when (key) {
        is ArmoredKey.Public -> throw Exception("Cannot use public key as private key")
