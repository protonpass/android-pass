...
@Singleton
class UpdateGlobalAliasAddressesMonitorStateImpl @Inject constructor(
    ...
    override suspend fun invoke(userId: UserId?, enabled: Boolean) {
        if (userId != null) {
            breachRepository.updateGlobalAliasMonitorState(userId, enabled)
        } else {
            val user = observeCurrentUser().filterNotNull().first()
            breachRepository.updateGlobalAliasMonitorState(user.userId, enabled)
        }
    }
}
