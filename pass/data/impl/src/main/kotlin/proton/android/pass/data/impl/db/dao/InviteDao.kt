...
@Dao
abstract class InviteDao : BaseDao<InviteEntity>() {
    @Query(
        """
        SELECT * FROM ${InviteEntity.TABLE}
        WHERE ${InviteEntity.Columns.USER_ID} = :userId
        ORDER BY ${InviteEntity.Columns.CREATE_TIME} ASC
        """
    )
    abstract fun observeAllForUser(userId: String): Flow<List<InviteEntity>>
