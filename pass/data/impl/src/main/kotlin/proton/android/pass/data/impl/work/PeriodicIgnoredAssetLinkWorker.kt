...
@HiltWorker
class PeriodicIgnoredAssetLinkWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result = runCatching {
        ...
        PassLogger.w(TAG, "Failed update ignored asset links")
        PassLogger.w(TAG, it)
    }.toWorkerResult()

    companion object {
        const val WORKER_UNIQUE_NAME = "periodic_ignore_asset_link_worker"
        private const val TAG = "PeriodicIgnoredAssetLinkWorker"
        private const val REPEAT_DAYS = 1L

        fun getRequestFor(): PeriodicWorkRequest =
            PeriodicWorkRequestBuilder<PeriodicIgnoredAssetLinkWorker>(REPEAT_DAYS, TimeUnit.DAYS)
                .setConstraints(
